...
class ProfileResponse(BaseModel):
    ...
    name: StrictStr = Field(description="Profile name property. The value obtained by file name for existing profiles. For new profiles the value is generated by a random name  generator.")
    tags: List[StrictStr] = Field(description="Profile tags")
    created_at: datetime = Field(description="Date when the profile was created.", alias="createdAt")
    fingerprint: Fingerprint = Field(description="Provides a full view of a fingerprint, which encapsulates real-world browser fingerprint configurations used to  instantiate virtual browser profiles.")
    ...
    timezone: TimezoneChoice = Field(description="Sets how the Timezone will be spoofed. Values can be 'automatic', 'manual', 'off'.")
    geolocation: GeolocationChoice = Field(description="Sets how the Geolocation will be spoofed. Values can be 'automatic', 'manual', 'block', 'off'.")
    proxy: ProxyChoice = Field(description="Proxy connection settings of the profiles. Values can be 'none', 'http', 'socks5', 'ssh'. When it is not set to none, a server is provided.")
    web_rtc: WebRtcChoice = Field(description="Sets how the WebRTC will be spoofed. Values can be 'automatic', 'manual', 'block', 'off'.", alias="webRtc")
    ...
    screen: ScreenChoice = Field(description="Sets how the Screen will be spoofed. Values can be 'automatic', 'manual', 'off'. When value is set to manual, a ScreenSize must be provided")
    hardware_concurrency: HardwareConcurrencyChoice = Field(description="Sets how the Hardware Concurrency will be spoofed. Values can be 'automatic', 'manual', 'off'. When value is set to manual, a  HardwareConcurrencyType must be provided (valid values:1, 2, 4, 8, 12, 16)", alias="hardwareConcurrency")
    device_memory: DeviceMemoryChoice = Field(description="Sets the level of device memory spoofing. Values can be 'automatic', 'manual', 'off'.  When value is set to manual, a specific amount of device memory must be provided (valid values: 0.25, 0.5, 1, 2, 4, 8, 16, 32)", alias="deviceMemory")
    language: StrictStr = Field(description="A language or a comma-separated list of languages for the profile.  ISO 639-1 language and optionally ISO 3166-1 region code.")
    start_page: StrictStr = Field(description="This website will be opened in the browser when the profile launches.", alias="startPage")
    ...
    extensions: List[StrictStr] = Field(description="A list of extensions or addons that will be loaded to the profile when the profile is started. For chrome and edge use CRX3 format  extensions. For firefox use signed xpi format addons.")
    notes: StrictStr = Field(description="A free text including any notes written by the user.")
    status: StatusResponse = Field(description="Status information about the profile.")
