...
class UpdateProfileRequest(BaseModel):
    ...
    folder_id: Optional[StrictStr] = Field(default=None, description="Id of the folder the profile should be moved to. If null it won't be changed, if empty (00000000-0000-0000-0000-000000000000) it will be moved to root.", alias="folderId")
    tags: Optional[List[StrictStr]] = Field(default=None, description="Profile tags")
    canvas: Optional[CanvasSpoofingType] = Field(default=None, description="Sets how the canvas will be spoofed. Values can be 'intelligent', 'noise', 'block', 'off'.")
    webgl: Optional[WebglSpoofingType] = Field(default=None, description="Sets how the WebGL will be spoofed. Values can be 'noise', 'block', 'off'.")
    webgl_meta: Optional[WebglMetaChoice] = Field(default=None, description="Sets how the WebGL Vendor and Renderer will be spoofed. Possible values:  'automatic': The vendor and renderer values comes from the fingerprint.  'manual': Manually set the vendor and renderer values.  'off': Turn off the spoofing, use the original settings", alias="webglMeta")
    audio: Optional[AudioSpoofingType] = Field(default=None, description="Sets how the Audio will be spoofed. Values can be 'noise', 'block', 'off'.")
    ...
    proxy: Optional[ProxyChoice] = Field(default=None, description="Proxy connection settings of the profiles. Values can be 'none', 'http', 'socks5', 'ssh'. When it is not set to none, a server must be  provided.")
    ...
    fonts: Optional[FontSpoofingType] = Field(default=None, description="Sets how the Fonts will be spoofed. Values can be 'enabled', 'disabled'.")
    ...
    hardware_concurrency: Optional[HardwareConcurrencyChoice] = Field(default=None, description="Sets how the Hardware Concurrency will be spoofed. Values can be 'automatic', 'manual', 'off'. When value is set to manual, a  HardwareConcurrencyType must be provided (valid values:1, 2, 4, 8, 12, 16)", alias="hardwareConcurrency")
    device_memory: Optional[DeviceMemoryChoice] = Field(default=None, description="Sets how the Device Memory will be spoofed. Values can be 'automatic', 'manual', 'off'. When value is set to manual, a DeviceMemoryType  must be provided (valid values: 0.25, 0.5, 1, 2, 4, 8, 16, 32)", alias="deviceMemory")
    language: Optional[StrictStr] = Field(default=None, description="A language or a comma-separated list of languages for the profile.  ISO 639-1 language and optionally ISO 3166-1 region code.")
    ...
    password_manager: Optional[PasswordManagerType] = Field(default=None, description="Indicates whether the browser supports saving login credentials.", alias="passwordManager")
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        * `None` is only added to the output dict for nullable fields that
        ...
        # set to None if canvas (nullable) is None
        # and model_fields_set contains the field
        if self.canvas is None and "canvas" in self.model_fields_set:
            _dict['canvas'] = None

        # set to None if webgl (nullable) is None
        # and model_fields_set contains the field
        if self.webgl is None and "webgl" in self.model_fields_set:
            _dict['webgl'] = None
        ...
        # set to None if password_manager (nullable) is None
        # and model_fields_set contains the field
        if self.password_manager is None and "password_manager" in self.model_fields_set:
            _dict['passwordManager'] = None
