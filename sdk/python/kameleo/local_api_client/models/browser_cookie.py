...
class BrowserCookie(BaseModel):
    ...
    """ # noqa: E501
    domain: StrictStr = Field(description="The domain attribute signifies the domain for which the cookie is valid and can be submitted with every request for this domain or its  subdomains. If this attribute is not specified, then the hostname of the originating server is used as the default value.")
    name: StrictStr = Field(description="The name for the Cookie.")
    path: StrictStr = Field(description="The path attribute indicates a URL path that must exist in the requested URL in order to send the Cookie header. The %x2F (\"/\") character  is considered a directory separator, and subdirectories match as well.")
